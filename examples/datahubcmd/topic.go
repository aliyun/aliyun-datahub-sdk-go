...
func init() {
	...
	CreateTopicCommand.StringVar(&Comment, "comment", "", "topic comment.")
...
func create_topic(dh *datahub.DataHub) error {
	...
	fmt.Printf("topic create suc! detail info:\n%s\n", t)
	return nil
}
...
func update_topic(dh *datahub.DataHub) error {
	err := dh.UpdateTopic(TopicName, ProjectName, Lifecycle, Comment)
	if err != nil {
		return err
	}
